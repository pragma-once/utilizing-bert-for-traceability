...
def nl_transform_row(row): # row to str
    text = ""
    title_exists = row["issue_title"] not in ["None", "nan", "NaN"]
    body_exists = row["issue_body"] not in ["None", "nan", "NaN"]
    if title_exists:
...
model.to(device) # Added line, to make the evaluation fast (use GPU).

#logger.info("model loaded")
start_time = time.time()
test_examples = load_examples(DATA_DIR_TEST, model=model, overwrite=args.overwrite,
                                num_limit=args.test_num, nl_transform=nl_transform_row, pl_transform=pl_transform_row)
